...
static void twai_monitor_task(void *arg) {
    ...
    while (1) {
        ...
        if (twai_read_alerts(&alerts, portMAX_DELAY) == ESP_OK) {
            ...
            // Recovery complete, safe to resume transmission
...
esp_err_t can_driver_init(gpio_num_t tx_pin, gpio_num_t rx_pin, uint32_t baud_rate) {
    ...
    // Start Core Tasks
    // Priorities matter here! Monitor should preempt everything else to handle faults.
    xTaskCreate(twai_monitor_task, "CAN_Mon", 2048, NULL, 6, NULL);
    xTaskCreate(tx_task,           "CAN_Tx",  4096, NULL, 5, NULL);
    xTaskCreate(dispatch_task,     "CAN_Rx",  4096, NULL, 5, NULL);
