 ...
 */
#define CAN_SUBSCRIBE_TOPIC(id, queue_ptr, queue_len) \
    do { \
...
static inline bool can_receive_payload(QueueHandle_t q, void* payload_ptr, size_t payload_size, uint32_t timeout_ms) {
    ...
    if (xQueueReceive(q, &rx_msg, pdMS_TO_TICKS(timeout_ms)) == pdTRUE) {
        // Safe copy: prevents buffer overruns if the received payload size mismatches the expected size
        size_t copy_len = (rx_msg.data_length_code < payload_size) ? rx_msg.data_length_code : payload_size;
        memcpy(payload_ptr, rx_msg.data, copy_len);
        return true; 
    }
    return false; 
}